...
def ReadArgs():
    ...
    parser.add_argument(
        "-sn", "--sName", help="SMS sender device login name. Default: admin",
        action="store", default="admin")
    parser.add_argument(
        "-sp", "--sPassword", help="SMS sender device login password. Default: Admin123",
        action="store", default="Admin123")
    parser.add_argument(
        "-sip", "--sAddress", help="SMS sender device IP address. Default: 192.168.1.1",
        action="store", default="192.168.1.1")
    parser.add_argument(
        "-rp", "--rPassword", help="SMS receiver device login password. Default: Admin123",
        action="store", default="Admin123")
    parser.add_argument(
        "-rip", "--rAddress", help="SMS receiver device IP address. Default: 192.168.1.2",
        action="store", default="192.168.1.2")
    parser.add_argument(
        "-file", "--configFile", help="Configuration file path. Default: ./event-config.json",
        action="store", default="event-config.json")
    parser.add_argument(
        "-d", "--deleteFile", help="Delete test result file from PC. Default: false",
